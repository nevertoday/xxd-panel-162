...
異なる元写真から Panel 162 で各1回生成した8点です。短い英文を使用し、横1536×864・左右均等、縦1152×1536・上下均等に整え、メタデータ清掃を検証しました。実際のモデル出力を残しており、全要件への適合を保証するものではありません。

確認された偏差：主題と白い切り紙の台紙が大きく余白が不足する例があり、風景の細部や追加装飾が多く残っています。
...
インストール後に Agent セッションを再起動し、`$xxd-panel-162` を呼び出します。ユーザー単位の Codex には `--global --agent codex --yes` を追加できます。

```text
/xxd-panel-162 photo.jpg --mode top-bottom --size 3:4 --text prompt --locale ja-JP
...
完全な実行契約は [SKILL.md](SKILL.md)、実行アダプターは[英語](references/xxd-panel-162-prompt.en.md)／[中国語](references/xxd-panel-162-prompt.zh-CN.md)を参照してください。
