...
# XXD Panel 162｜رسم قصصي على قصاصة ورق بيضاء
...
وُلّدت هذه النماذج الثمانية بواسطة Panel 162، كل منها بمحاولة واحدة ومن صورة مصدر مختلفة مع نص إنجليزي قصير. ضُبطت الأبعاد إلى 1536×864 بتقسيم متساوٍ يميناً ويساراً و1152×1536 بتقسيم متساوٍ أعلى وأسفل، وتم التحقق من تنظيف البيانات الوصفية. هذه نتائج فعلية وليست تأكيداً لاجتياز جميع متطلبات الأسلوب.

انحرافات ملحوظة: بعض العناصر وخلفيات القصاصات البيضاء كبيرة فتقل المساحات الفارغة، وتبقى تفاصيل بيئية وزخارف إضافية في بعض المشاهد.
...
رسم قصصي عفوي محبب الملمس فوق قصاصة بيضاء واحدة متفاوتة العرض، لا خط أبيض منتظم حول الموضوع.
...
- `design-only`: تعرض اللوحة الكاملة ترجمة Panel 162 التصميمية فقط، وتبقى الصورة مرجعاً غير ظاهر.
...
بعد التثبيت أعد تشغيل جلسة Agent ثم استدعِ `$xxd-panel-162`. ويمكن إضافة `--global --agent codex --yes` للتثبيت على مستوى المستخدم.

```text
/xxd-panel-162 photo.jpg --mode top-bottom --size 3:4 --text prompt --locale ar-SA
...
راجع [SKILL.md](SKILL.md) لعقد التشغيل الكامل، ومهايئ التشغيل [بالإنجليزية](references/xxd-panel-162-prompt.en.md) أو [بالصينية](references/xxd-panel-162-prompt.zh-CN.md).
